...
import com.eagle.micro.job.detail.RestCallJobDetail;
...
@Component
@Slf4j
public class ProcCallJobManager extends JobManager {

	private static final String PARAM_KEY = "url";
	...
	public void scheduleJob(QrtzJob job) throws SchedulerException {
		...
		if (!scheduler.checkExists(jobKey)) {
			// 启动job
			JobDetail jobDetail = JobBuilder.newJob(RestCallJobDetail.class).withIdentity(jobKey)
			...
			log.warn("创建接口【" + job.getJobDataMap().get(PARAM_KEY) + "】调度job");
		}
